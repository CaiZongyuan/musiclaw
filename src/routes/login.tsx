...
function LoginRoute() {
  ...
  const { clearSession, profile, rawCookie, setLoginMode, setSession } = useAuthStore(
    useShallow((state) => ({
  ...
  async function finalizeLogin(cookie: string | undefined, loginMode: 'account') {
    if (!cookie) {
  ...
  const accountMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      try {
        await finalizeLogin(result.cookie, 'account')
      } catch (error) {
    ...
    () => [
      {
        key: 'qr' as const,
        title: '二维码登录',
        description: '最接近原版 YesPlayMusic 的登录方式',
        icon: QrCode,
      },
      {
        key: 'phone' as const,
        title: '手机号登录',
        description: '直接使用网易云手机号与密码登录',
        icon: Smartphone,
      },
      {
        key: 'email' as const,
        title: '邮箱登录',
        description: '支持网易邮箱账号登录',
        icon: Mail,
      },
    ],
    [],
      ...
      <main className="py-10">
        <section className="island-shell rounded-[2rem] p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button type="button" onClick={() => void navigate({ to: '/library' })} className="app-chip cursor-pointer">
              进入音乐库
            </button>
    ...
    <main className="login-screen py-10">
      <section className="login-shell island-shell rounded-[2rem] p-6 sm:p-8">
        <div className="login-shell__hero">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--sea-ink-soft)] sm:text-base">
            这轮已经不再是说明页，而是可用登录页。优先保持 YesPlayMusic 的网易云登录模型，不切到 `better-auth` 主流程。
          </p>
        </div>
        ...
        {mode === 'qr' ? (
          ...
          <section className="login-panel mt-8">
              ...
              {mode === 'phone' ? (
                <>
                  <label className="login-field">
                    <span>国家区号</span>
                    <input value={countryCode} onChange={(event) => setCountryCode(event.target.value)} placeholder="+86" />
                  </label>
                  <label className="login-field">
                    <span>手机号</span>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <button type="button" onClick={handleAccountLogin} disabled={isBusy} className="app-chip cursor-pointer disabled:cursor-not-allowed disabled:opacity-50">
                {isBusy ? '登录中…' : '登录'}
              </button>
              <button type="button" onClick={() => void navigate({ to: '/' })} className="app-chip cursor-pointer">
                返回首页
              </button>
            </div>
          </section>
