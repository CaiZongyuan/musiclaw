...
- Done:
  ...
  - 完成 Phase 3 播放器控制层第一批交互：底部播放器支持进度拖动、循环模式切换、随机播放切换、音量调节百分比展示，并根据当前播放进度滚动歌词预览
  - 修复 `player-store` 在切歌 / seek 时的时长与进度边界处理，避免切到下一首后仍残留上一首时长或 seek 超出时长
  - 完成首页、搜索页、歌单页、专辑页、艺人页及根路由的共享错误态接入：loader / 渲染异常时会显示可恢复的错误页面，而不是直接白屏
  - 完成 Phase 3 歌词体验第一批实现：底部播放器支持展开滚动歌词面板、按当前播放时间高亮当前句，并展示时间对齐的翻译歌词
  - 在 `track/lib/lyrics.ts` 中抽出当前歌词定位与预览 helper，并补充歌词单测
- In progress:
  - Phase 3 歌词体验：继续评估独立歌词页、逐字歌词和更多播放反馈
- Next:
  - 为歌词面板补充更多交互（如固定打开状态、跳转到当前句、移动端高度适配）
  - 为播放器补充更多单测（如上一首/随机播放边界）与组件交互测试
  - 视需要为更多业务路由补 `notFound` / `errorComponent` 细分状态
- Blockers:
