...
- Done:
  - `src/routes/new-album.tsx` 已把头部从说明式 hero 收回到更接近原版的 `大标题 + 区域切换 + 卡片流` 结构，减少说明文案比重
  - `src/styles.css` 已重做 `New Album` 页面样式：桌面端恢复 5 列封面流节奏，收紧卡片间距、标题字号、说明文字密度与播放按钮显隐方式
  - `New Album` 卡片已改为更接近首页 / 原版 `CoverRow` 的视觉：透明卡片外壳、较小圆角封面、hover 放大和标题高亮
  - 已清理此前重复追加导致的 `New Album` 重复样式块，避免后续样式维护继续漂移
  - 已运行 `./node_modules/.bin/tsc --noEmit`，当前 TypeScript 校验通过
- In progress:
  - 等待真实浏览器手测，确认 `New Album` 在桌面端和移动端的卡片密度、标题截断和播放按钮显隐都更接近原版
- Next:
  - 根据你的实机反馈继续微调 `New Album` 的标题字号、卡片间距和分页控件位置
  - 若这轮样式稳定，再回头继续收其它主路径页的视觉细节
- Blockers:
  - 当前 `New Album` 仍保留了区域切换与分页控制，和旧版单页长列表并不完全一致；若后续要追更细 parity，可再评估是否回退到更原版的长列表方案
